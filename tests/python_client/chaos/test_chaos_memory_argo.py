...
class TestMilvusChaos:
    ...
    def test_create_index(self, collection_name):
        ...
        try:
            ...
            self.collection_w.create_index(field_name=ct.default_float_vec_field_name,
                                           index_params=index_params, timeout=60)
    ...
    def test_init_collection_check(self, collection_name, replica_num):
        ...
        self.collection_w.create_index(field_name=ct.default_float_vec_field_name, index_params=index_params)
        ...
        try:
            ...
            while time.time() - start < search_duration:
                ...
                self.collection_w.query(f"{ct.default_int64_field_name} in {search_res[0].ids}",
                                        check_task=CheckTasks.check_query_empty)
        except MilvusException as e:
            raise MilvusException(e)

        self.collection_w.compact()
        self.collection_w.wait_for_compaction_completed(timeout=60)
        self.collection_w.get_compaction_plans(timeout=60)


class TestDebug:
    def test_insert(self, host, port):
        # mc = MilvusChaos()
        # mc.init_collection(host=host, port=port)
        # mc.do_insert()
        # mc.do_flush()
        pass


class TestStandaloneChaos:
    def test_standalone_while_insert(self):
        """
        1. init collection
        2. insert while chaos memory and insert failed
        3. wait all pods running
        4. flush and load search
        """
        pass

    def test_standalong_while_index(self):
        """
        1. init collection
        2. create index while while chaos
        3. wait all pods running
        4. load and search
        """
        pass

    def test_standalone_while_load(self):
        """
        1. init collection
        2. load collection while chaos and expected exception
        3. wait all pods running
        4. load and search
        """
        pass
...
class TestClusterChaos:
    ...
    def test_datanode_while_flush(self):
        """
